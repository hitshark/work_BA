...
//
  input                       pause;
//
// From the arbiter_2t, inidcator as to who is granted the bus
//
  input [`NUM_AHB_MASTERS:0]   grant_2t;
//
// From the arbiter_2t, inidcator as to idle bus
//
  input                        parked_2t;
//
...
// current masters hlock which is next hlock register input
  reg                          ihlock_nxt;
// Update the ihlock register and hold during wait states or
// locked split transfers
  reg                          ihlock_en;
// Aligned to data
  reg                          ihlock;
// Current state
...
// Previous transfer was a split transfer
  reg                          ptwst;
...
//
  always @(bus_hlock or nxt_hmaster or ihlock_en) 
...
//
// There are times when one does not update the ihlock register. One
// being that there is a split transaction at present and also at any
// time when a slave is holding off a master with the hready low. At
// these times need to hold onto the current value.
//
  always @(hready or lock_sm)
  begin : ihlock_en_PROC
    if (lock_sm == S) begin
      ihlock_en = 1'b0;
    end else begin
      ihlock_en = hready;
...
//
// Want to know if we previously had a split to a locked transfer.
// Whenever the locked state machine is within its S-state, or split
// state then one knows we have had a split transfer.
//
  always @(posedge hclk or negedge hresetn)
  begin : ptwst_PROC
    if (hresetn == 1'b0)
    ...
    if (hresetn == 1'b0)
      ...
      if (uhgrant == 1'b1) begin
        if (((nslt == 1'b1) && (hresp == `SPLIT) && (hready == 1'b0)) || 
          ...
          if (((lock_sm == LL) && (hresp == `RETRY))) begin
...
//
  ...
  always @(lock_sm or hready or ihlock_nxt or ihlock or htrans or
           ptwst or hresp or nxt_hmaster or hmaster_d or csilt or hmaster)
