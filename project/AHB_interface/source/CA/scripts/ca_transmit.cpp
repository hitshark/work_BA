...
void ca_transmit::transmit_thread(void)
{
    ...
    //PERL:RESET: reset all the output signals and local variables
    hbusreq_o=FALSE;
    hsize_o=WORD;
    htrans_o=IDLE;
    hburst_o=SINGLE;
    hwrite_o=READ;
    haddr_o=DEFAULT_ADDR;
    hwdata_o=DEFAULT_DATA;
	//PERL:END RESET: flag being used in perl scripts
    busy_flag=0;
        ...
        switch(ahb_master_state)
        {
            ...
            case AHBM_BREQ:
            {
                if(hgrant_i.read() & hready_i.read())
                {
                    ...
                    hburst_o=t_burst;
                    hsize_o=t_size;
                    htrans_o=NONSEQ;                    // transmit type is set to nonsequential
                ...
                if(hready_i.read() & (hresp_i.read()== OKAY))
                {
                    ...
                    else
                    {
                        ...
                        else if(beat_count==1)
                        {
                            ahb_master_state=AHBM_RDWAIT;
							//PERL:BRQ_TR_ADR
							hbusreq_o=FALSE;                     // at the end edge of the address phase
							htrans_o=IDLE;
							haddr_o=DEFAULT_ADDR;
							//PERL:END BRQ_TR_ADR
                        }
         ...
         // master rearbitrates and starts with non-sequential again
            ...
            case AHBM_NSEQWR:
            {
                ...
                // is last data phase
                {
					...
					{
						//PERL:WDATA_TR_ADR
						hwdata_o=t_wdata[beat_count-1];     // send write data from write data buffer
						htrans_o=IDLE;
						haddr_o=DEFAULT_ADDR;
						//PERL:END WDATA_TR_ADR
                        ahb_master_state=AHBM_LASTWR;
						break;
					}
            ...
            case AHBM_SEQWR:
            {
                if(hready_i.read() && hresp_i.read()==OKAY)
                {
                    ...
                    else
                    {
                        ...
                        {
							//PERL:WDATA_TR_ADR_BRQ
							hwdata_o=t_wdata[beat_count-1];
							htrans_o=IDLE;
							haddr_o=DEFAULT_ADDR;
							hbusreq_o=FALSE;                 // at the end edge of the address phase
							//PERL:END WDATA_TR_ADR_BRQ
                            ahb_master_state=AHBM_WRWAIT;
                        }
                    }
                }
